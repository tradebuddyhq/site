...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $item_type = $conn->real_escape_string($_POST['item_type']); // 'Lost' or 'Found'
    $item_name = $conn->real_escape_string($_POST['item_name']);
    $category = $conn->real_escape_string($_POST['category']);
    $location = $conn->real_escape_string($_POST['location']);
    $date_reported = $conn->real_escape_string($_POST['date_lost'] ?? $_POST['date_found']);
    ...
    if ($stmt->execute()) {
        echo "<script>alert('Report submitted successfully!'); window.location.href = 'lost-found.php';</script>";
    } else {
